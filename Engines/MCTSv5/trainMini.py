...
# Assuming the model outputs are logits, convert them to probabilities
def apply_label_smoothing_to_loss(logits, labels, eps=0.1):
    # Convert logits to probabilities using softmax
    lprobs = F.log_softmax(logits, dim=-1)

    # Apply label smoothing to the target labels
    return label_smoothed_nll_loss(lprobs, labels, eps)
...
class MCTS_Train:
    ...
    def train_supervised(self):
        ...
        for i in range(start_epoch, start_epoch + 50):
            ...
            for batch in tqdm(data_loader):
                ...
                alpha = 0.75
                loss = criterion_move(model_from, label_from)*(1-alpha)*2 + criterion_move(model_to, label_to)*alpha*2
